...
function renderUserLayout() {
    ...
    app.innerHTML = `
        <div class="flex h-full w-full">
            <!-- SIDEBAR -->
            <aside class="sidebar fixed inset-y-0 left-0 w-72 bg-panel border-r border-border transform -translate-x-full md:translate-x-0 z-40 flex flex-col transition-transform duration-300">
            ...
            <!-- MAIN CONTENT -->
            <main id="user-content" class="flex-1 md:ml-72 bg-dark h-full flex flex-col overflow-hidden relative"></main>
        </div>
    `;
    lucide.createIcons();
...
async function loadChat() {
    ...
    container.innerHTML = `
        <div class="flex flex-col h-full bg-dark">
            <!-- Header -->
            <div class="h-16 border-b border-border bg-panel flex items-center px-6 shadow-sm z-10 flex-shrink-0">
            ...
            <!-- Input Area -->
            <div class="p-4 bg-panel border-t border-border flex-shrink-0">
                <div class="relative flex items-center gap-2 max-w-4xl mx-auto">
                    <input id="msg-input" type="text" placeholder="Transmit secure message..." autocomplete="off"
                        class="w-full bg-black/30 border border-border text-white px-5 py-3 rounded-full focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder-gray-600">
                    
                    <button class="w-12 h-12 bg-primary hover:bg-primary-hover text-white rounded-full flex items-center justify-center shadow-lg shadow-primary/20 transition-transform active:scale-95" onclick="window.sendMessage()">
...
function renderMessages(msgs, append = false) {
    ...
    const html = msgs.map(m => `
        <div class="flex flex-col ${m.user_id === currentUser.id ? 'items-end' : 'items-start'} animate-fade-in">
            <div class="text-[10px] uppercase font-bold text-gray-500 mb-1 px-1 tracking-wider">${m.profiles?.full_name || 'Unknown'}</div>
            <div class="max-w-[85%] md:max-w-[70%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm ${m.user_id === currentUser.id ? 'bg-primary text-white rounded-br-none' : 'bg-card border border-border text-gray-200 rounded-bl-none'}">
...
async function loadUserTasks() {
    ...
    container.innerHTML = `
        <div class="p-6 md:p-8 overflow-y-auto h-full">
            <h2 class="font-heading font-bold text-3xl text-white mb-6 tracking-wide">MY OBJECTIVES</h2>
            <div id="task-list" class="grid grid-cols-1 gap-4 max-w-4xl">
    ...
    list.innerHTML = tasks.map(t => {
        ...
        <div class="bg-card border border-border rounded-xl p-5 relative overflow-hidden group hover:border-gray-600 transition-colors shadow-lg">
            ...
            <div class="flex justify-between items-start mb-3">
                ...
                <span class="text-xs font-bold uppercase tracking-widest px-2 py-1 bg-black/30 rounded ${statusColor}">${t.status}</span>
            </div>
